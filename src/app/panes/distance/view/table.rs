        ...
        match (row, column) {
            ...
            (0, top::DEAD_TIME) => {
                ui.heading(ui.localize(DEAD_TIME))
                    .on_hover_ui(|ui| {
                        ui.label(ui.localize(formatcp!("{DEAD_TIME}.abbreviation")));
                    })
                    .on_hover_ui(|ui| {
            ...
            (1, bottom::FROM) => {
                ui.heading(ui.localize("From")).on_hover_ui(|ui| {
                    ui.label(ui.localize("From.hover"));
                });
            }
            (1, bottom::TO) => {
                ui.heading(ui.localize("To")).on_hover_ui(|ui| {
                    ui.label(ui.localize("To.hover"));
                });
            }
            (1, bottom::RETENTION_TIME) => {
                ui.heading(ui.localize(formatcp!("{RETENTION_TIME}.abbreviation")))
                    .on_hover_ui(|ui| {
                        ui.label(ui.localize(RETENTION_TIME));
                    });
            }
