...
def show_stats_window(stats_data):
    ...
    stats_win.title("Статистика линейной регрессии")
    stats_win.geometry("800x300")
    
    # Создаем таблицу
    columns = ("curve", "equation", "r2", "p_value", "std_err")
    tree = ttk.Treeview(stats_win, columns=columns, show="headings")
    
    tree.heading("curve", text="Кривая")
    tree.heading("equation", text="Уравнение (y = mx + c)")
    tree.heading("r2", text="R² (Точность)")
    tree.heading("p_value", text="p-value")
    tree.heading("std_err", text="Стандартная ошибка")
    
    tree.column("curve", width=100, anchor="center")
    tree.column("equation", width=250, anchor="center")
    tree.column("r2", width=100, anchor="center")
    tree.column("p_value", width=150, anchor="center")
    tree.column("std_err", width=150, anchor="center")
    
    for stat in stats_data:
        tree.insert("", tk.END, values=(
            stat['curve'],
            f"y = {stat['slope']:.4f}x + {stat['intercept']:.4f}",
            f"{stat['r2']:.4f}",
            f"{stat['p_value']:.2e}",
            f"{stat['std_err']:.4f}"
        ))
...
def plot_graph():
    ...
    if plot_type == "Line":
        ...
        for val in sorted(filtered_df[hue_col].unique()):
            ...
            if do_regression and len(x_data) > 1:
                ...
                y_fit = res.intercept + res.slope * x_data
                plt.plot(x_data, y_fit, color=color, linestyle='-', label=f'{val} (регр. R²={r_squared:.3f})')
...
ttk.Checkbutton(frame_reg, text="Добавить линейную регрессию и статистику (только для линий)", variable=var_regression).pack(anchor="w")
