...
def start_analysis():
    ...
    def refresh_plots(val=None):
        """Основная функция перерисовки с учетом диапазона и режима"""
        ...
        if mode == 'Нормированный':
            ...
            for item in plots_registry:
                # Находим данные в видимом диапазоне
                ...
                if len(visible_y) > 0 and np.max(visible_y) > 0:
                    ...
                    # Нормируем ВЕСЬ график по локальному максимуму видимой части
                    item['line'].set_ydata((item['y_orig'] / local_max) * 100)
                else:
                    # Если данных в окне нет, просто зануляем для корректности
                    item['line'].set_ydata(item['y_orig'] * 0)
            
            ax.set_ylim(-2, 105) # Фиксируем Y для %
        else:
            ax.set_ylabel('Интенсивность (Counts)')
            max_y_visible = 0
            for item in plots_registry:
                item['line'].set_ydata(item['y_orig'])
                # Считаем макс. видимый Y для автомасштаба в абсолютном режиме
                mask = (item['t'] >= x_min) & (item['t'] <= x_max)
                visible_y = item['y_orig'][mask]
                if len(visible_y) > 0:
    ...
    def on_text_submit(text):
        try:
            ...
            slider.set_val((v_min, v_max)) # Это само вызовет refresh_plots
        except: pass
    ...
    def add_files(event):
        ...
        if plots_registry:
            ...
            # Сброс на полный вид при добавлении
