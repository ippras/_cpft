...
class ChromatogramApp:
    ...
    def create_control_window(self):
        ...
        self.ctrl_win.geometry("800x900")
    ...
    def setup_tab_helpers(self):
        ...
        f_add = ttk.LabelFrame(frame, text="Новая линия", padding=10); f_add.pack(fill="x")
        ...
        ttk.Button(f_add, text="➕ Создать", command=self.add_helper).grid(row=0, column=2, padx=10)
    ...
    def setup_tab_style(self):
        ...
        for i, (l, p, f, t) in enumerate(rows):
            self.create_style_row(frame, l, p, f, t, i+1)
    ...
    def update_ui_list(self):
        ...
        for i, item in enumerate(reversed(self.plots_registry)):
            ...
            sp = tk.Spinbox(f, from_=0.1, to=10, increment=0.1, width=4, command=lambda ix=idx: self.edit_plot(ix, 'line_width', None)); sp.delete(0, "end"); sp.insert(0, f"{item['line_width']:.1f}"); sp.pack(side="left", padx=2)
            item['width_spin'] = sp
    ...
    def add_helper(self):
        ...
        self.helper_lines.append({'type': self.h_type.get(), 'pos': round(np.mean(x_l if self.h_type.get()=='V' else y_l), 2), 'lim': round(y_l[1] if self.h_type.get()=='V' else x_l[1], 2), 'color': '#FF0000'})
    ...
    def refresh_plots(self):
        ...
        for h in self.helper_lines:
            if h['type'] == 'V':
                l = self.ax.vlines(h['pos'], 0, h['lim'], colors=h['color'], zorder=5)
                t = self.ax.text(h['pos'], h['lim'], f" {h['pos']}", color=h['color'], fontsize=8, va='bottom')
            else:
                l = self.ax.hlines(h['pos'], self.ax.get_xlim()[0], h['lim'], colors=h['color'], zorder=5)
                t = self.ax.text(h['lim'], h['pos'], f" {h['pos']}", color=h['color'], fontsize=8, ha='left')
    ...
    def reset_all(self):
        self.style_params = self.default_style.copy(); self.grid_var.set(True); self.legend_var.set(True)
        self.update_ui_list(); self.refresh_plots()
    def manual_fit(self):
