...
class ChromatogramApp:
    ...
    def update_helper_ui(self):
        ...
        for i, h in enumerate(self.helper_lines):
            ...
            # Цвет и удаление
            btn_c = tk.Button(f_row, bg=h['color'], width=2, relief="flat", command=lambda idx=i: self.pick_helper_color(idx)).pack(side="left", padx=2)
    ...
    def update_helper_increments(self):
        ...
        for h in self.helper_lines:
            try:
                ...
                h['pos_spin'].config(increment=step_main)
                h['label_offset_spin'].config(increment=step_main)
                h['start_spin'].config(increment=step_range)
                h['end_spin'].config(increment=step_range)
                # Для процентов всегда шаг 1
                h['label_pos_spin'].config(increment=1)
            except:
                pass
    ...
    def edit_helper_spin(self, idx, key):
        try:
            # Получаем значение, заменяя запятую на точку
