...
class ChromatogramApp:
    ...
    def setup_tab_style(self):
        ...
        # --- ШРИФТЫ ---
        ttk.Label(f, text="ОБЩИЕ НАСТРОЙКИ ШРИФТА", font='Arial 10 bold').grid(row=0, column=0, columnspan=3, sticky="w", pady=(0,10))
        self.create_font_row(f, "Общий шрифт", "gen", 1, include_inherit=False)

        ttk.Separator(f, orient="horizontal").grid(row=2, column=0, columnspan=3, sticky="ew", pady=15)
        ttk.Label(f, text="ИНДИВИДУАЛЬНЫЕ НАСТРОЙКИ", font='Arial 10 bold').grid(row=3, column=0, columnspan=3, sticky="w")
        
        self.create_font_row(f, "Заголовок", "title", 4, size_range=(8, 30))
        self.create_font_row(f, "Подписи осей", "label", 6, size_range=(8, 24))
        self.create_font_row(f, "Числа на осях", "tick", 8, size_range=(6, 20))
        self.create_font_row(f, "Легенда", "legend", 10, size_range=(6, 20))

        # --- ЛИНИИ И СЕТКА ---
        ttk.Separator(f, orient="horizontal").grid(row=12, column=0, columnspan=3, sticky="ew", pady=15)
        style_rows = [
    ...
    def update_style(self, event=None):
        ...
        # Оси
        self.ax.set_xlabel(p['x_label'], fontfamily=self.get_font_prop('label', 'font'),
                           fontweight=self.get_font_prop('label', 'weight'),
                           fontstyle=self.get_font_prop('label', 'style'), fontsize=p['label_size'])
        
        self.ax.set_ylabel(self.ax.get_ylabel(), fontfamily=self.get_font_prop('label', 'font'),
                           fontweight=self.get_font_prop('label', 'weight'),
                           fontstyle=self.get_font_prop('label', 'style'), fontsize=p['label_size'])
        ...
        self.ax.grid(False)
        self.ax.grid(visible=True, which='major', alpha=p['grid_alpha'], lw=p['grid_width'])
