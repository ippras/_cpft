...
class ChromatogramApp:
    def __init__(self, root):
        ...
        # Устанавливаем фиксированные отступы, чтобы подписи не пропадали
        self.fig.subplots_adjust(bottom=0.15, left=0.12, top=0.92, right=0.95)
    ...
    def setup_tab_style(self):
        ...
        # --- ШРИФТЫ ---
        ttk.Label(f, text="ОБЩИЕ НАСТРОЙКИ ШРИФТА", font='Arial 10 bold').grid(row=1, column=0, columnspan=3, sticky="w")
        ...
        # --- ЛИНИИ И СЕТКА ---
        ttk.Separator(f, orient="horizontal").grid(row=13, column=0, columnspan=3, sticky="ew", pady=15)
        style_rows = [
    ...
    def update_style(self, event=None):
        ...
        self.ax.set_ylabel(y_label_text, fontfamily=self.get_font_prop('label', 'font'),
                           fontweight=self.get_font_prop('label', 'weight'),
                           fontstyle=self.get_font_prop('label', 'style'), fontsize=p['label_size'])
    ...
    def apply_params_to_ui(self):
        """Синхронизирует все виджеты интерфейса с текущим словарем style_params"""
        ...
        for p, w in self.style_widgets.items():
            if p in self.style_params:
                ...
                w['scale'].set(val)
                w['entry'].delete(0, tk.END); w['entry'].insert(0, f"{val:.1f}")
        ...
        for p, w in self.tick_widgets.items():
            if p in self.style_params:
                val = self.style_params[p]
                w['scale'].set(val)
                w['entry'].delete(0, tk.END); w['entry'].insert(0, f"{val:.3f}")
        
        # Текстовые подписи
        for k, en in self.label_entries.items():
            if k in self.style_params:
    ...
    def import_style(self):
        ...
        if path:
            try:
                ...
                self.style_params.update(new_s)
                self.apply_params_to_ui()
