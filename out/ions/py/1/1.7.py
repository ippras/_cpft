...
class ChromatogramApp:
    def __init__(self, root):
        ...
        self.default_style = {
            ...
            'legend_size': 10.0, 'line_width': 1.5, 'grid_alpha': 0.3, 'grid_width': 0.8,
            'spine_width': 1.0, 
    ...
    def setup_tab_style(self):
        ...
        # Размеры и толщины
        rows = [
            ("Заголовок", 'title_size', 8, 24), ("Оси", 'label_size', 8, 20), ("Числа", 'tick_size', 6, 16), 
            ...
            ("Сетка (толщ)", 'grid_width', 0.1, 2), ("Сетка (прозр)", 'grid_alpha', 0, 1)
        ]
        
        curr_row = 1
        for l, p, f, t in rows:
            self.create_style_row(frame, l, p, f, t, curr_row)
            curr_row += 1

        ttk.Separator(frame, orient="horizontal").grid(row=curr_row, column=0, columnspan=3, sticky="ew", pady=10)
        curr_row += 1

        # Шаги делений
        ttk.Label(frame, text="Шаг делений (0 = Авто):", font='Arial 9 bold').grid(row=curr_row, column=0, sticky="w")
        curr_row += 1
        
        tick_configs = [("X Основной", 'x_major_step', 10), ("Y Основной", 'y_major_step', 1000000),
                        ("X Промежут.", 'x_minor_step', 5), ("Y Промежут.", 'y_minor_step', 500000)]
        
        for label, param, limit in tick_configs:
            ttk.Label(frame, text=label+":").grid(row=curr_row, column=0, sticky="w")
            s = ttk.Scale(frame, from_=0, to=limit, command=lambda e, p=param: self.on_tick_slider(p, e))
            s.set(self.style_params[param]); s.grid(row=curr_row, column=1, sticky="ew", padx=5)
            e = ttk.Entry(frame, width=8); e.insert(0, "0.0"); e.grid(row=curr_row, column=2)
            e.bind("<Return>", lambda ev, p=param: self.on_tick_entry(p))
            self.tick_widgets[param] = {'scale': s, 'entry': e}
            curr_row += 1

        ttk.Button(frame, text="🔄 Сбросить всё", command=self.reset_all).grid(row=curr_row, column=0, columnspan=3, sticky="ew", pady=10)
    ...
    def on_style_entry(self, p):
        try:
            ...
            self.style_widgets[p]['scale'].set(v)
            self.style_params[p] = v
            self.refresh_plots()
        except: pass

    def on_tick_slider(self, param, val):
        if not self.ui_ready: return
        v = float(val)
        self.style_params[param] = v
        ent = self.tick_widgets[param]['entry']
        ent.delete(0, tk.END); ent.insert(0, f"{v:.3f}")
        self.refresh_plots()

    def on_tick_entry(self, param):
        try:
            v = float(self.tick_widgets[param]['entry'].get())
            self.tick_widgets[param]['scale'].set(v)
            self.style_params[param] = v
            self.refresh_plots()
        except: pass
    ...
    def refresh_plots(self):
        ...
        if self.mode_var.get() == "Нормированный":
            y_lbl = self.style_params['y_label_norm']
        ...
        self.update_style(); self.canvas.draw_idle()
    ...
    def update_style(self, event=None):
        ...
        # Настройка делений
        self.ax.tick_params(which='major', labelsize=p['tick_size'], width=p['major_tick_width'], length=p['major_tick_length'])
