...
class ChromatogramApp:
    ...
    def create_control_window(self):
        ...
        self.ctrl_win.geometry("1100x950")
    ...
    def setup_tab_style(self):
        container = ttk.Frame(self.tab_style)
        ...
        f = ttk.Frame(scroll_frame, padding=20); f.pack(fill="both")
        f.columnconfigure(1, weight=1)

        # Кнопки Импорта/Экспорта
        io_frame = ttk.Frame(f)
        io_frame.grid(row=0, column=0, columnspan=3, sticky="ew", pady=(0,15))
        ttk.Button(io_frame, text="💾 Экспорт стиля (JSON)", command=self.export_style).pack(side="left", expand=True, fill="x", padx=2)
        ttk.Button(io_frame, text="📂 Импорт стиля (JSON)", command=self.import_style).pack(side="left", expand=True, fill="x", padx=2)

        # --- ШРИФТЫ ---
        ttk.Label(f, text="ОБЩИЕ НАСТРОЙКИ ШРИФТА", font='Arial 10 bold').grid(row=1, column=0, columnspan=3, sticky="w", pady=(10,10))
        self.create_font_row(f, "Общий шрифт", "gen", 2, include_inherit=False)

        ttk.Separator(f, orient="horizontal").grid(row=4, column=0, columnspan=3, sticky="ew", pady=15)
        ttk.Label(f, text="ИНДИВИДУАЛЬНЫЕ НАСТРОЙКИ", font='Arial 10 bold').grid(row=5, column=0, columnspan=3, sticky="w")
        
        self.create_font_row(f, "Заголовок", "title", 6, size_range=(8, 30))
        self.create_font_row(f, "Подписи осей", "label", 8, size_range=(8, 24))
        self.create_font_row(f, "Числа на осях", "tick", 10, size_range=(6, 20))
        self.create_font_row(f, "Легенда", "legend", 12, size_range=(6, 20))

        # --- ЛИНИИ И СЕТКА ---
        ttk.Separator(f, orient="horizontal").grid(row=14, column=0, columnspan=3, sticky="ew", pady=15)
        style_rows = [
            ("Линии графиков", 'line_width', 0.5, 5),
        ...
        r_idx += 1
        ttk.Label(f, text="ШАГ ДЕЛЕНИЙ (0 = Авто)", font='Arial 10 bold').grid(row=r_idx, column=0, columnspan=3, sticky="w")
        r_idx += 1
        ...
        for lbl, p, lim in tick_rows:
            ...
            e = ttk.Entry(f, width=8); e.insert(0, "0.0"); e.grid(row=r_idx, column=2)
            e.bind("<Return>", lambda ev, param=p: self.on_tick_entry(param))
            self.tick_widgets[p] = {'scale': s, 'entry': e}
            r_idx += 1

        ttk.Button(f, text="🔄 Сбросить всё в дефолт", command=self.reset_all).grid(row=r_idx, column=0, columnspan=3, sticky="ew", pady=20)
    ...
    def create_font_row(self, master, label, prefix, row, include_inherit=True, size_range=None):
        ...
        if include_inherit:
            families, weights, styles = [["Inherit"] + x for x in [families, weights, styles]]
        ...
        for prop, vals, w in [('font', families, 15), ('weight', weights, 10), ('style', styles, 10)]:
            ...
            var.trace_add("write", lambda *a: self.refresh_plots())
        ...
        if size_range:
            ...
            e = ttk.Entry(f_frame, width=4); e.insert(0, f"{self.style_params[p_name]:.1f}"); e.pack(side="left")
            e.bind("<Return>", lambda ev: self.on_style_entry(p_name))
    ...
    def create_style_row(self, master, label, param, f, t, row):
        ...
        e.bind("<Return>", lambda ev: self.on_style_entry(param))
    ...
    def on_style_entry(self, p):
        try:
            v = float(self.style_widgets[p]['entry'].get())
            self.style_widgets[p]['scale'].set(v); self.style_params[p] = v; self.refresh_plots()
        except: pass

    def on_tick_slider(self, p, v):
        if not self.ui_ready: return
        val = float(v); self.style_params[p] = val
        self.tick_widgets[p]['entry'].delete(0, tk.END); self.tick_widgets[p]['entry'].insert(0, f"{val:.3f}")
        self.refresh_plots()

    def on_tick_entry(self, p):
        try:
            v = float(self.tick_widgets[p]['entry'].get())
            self.tick_widgets[p]['scale'].set(v); self.style_params[p] = v; self.refresh_plots()
        except: pass
    ...
    def update_style(self, event=None):
        ...
        # Заголовок
        self.ax.set_title(p['title_text'], fontfamily=self.get_font_prop('title', 'font'),
                          fontweight=self.get_font_prop('title', 'weight'),
                          fontstyle=self.get_font_prop('title', 'style'), fontsize=p['title_size'])
        
        # Оси
        for ax_obj in [self.ax.xaxis, self.ax.yaxis]:
            ax_obj.label.set_fontfamily(self.get_font_prop('label', 'font'))
            ax_obj.label.set_fontweight(self.get_font_prop('label', 'weight'))
            ax_obj.label.set_fontstyle(self.get_font_prop('label', 'style'))
            ax_obj.label.set_fontsize(p['label_size'])
            for tick in ax_obj.get_ticklabels():
                tick.set_fontfamily(self.get_font_prop('tick', 'font'))
                tick.set_fontweight(self.get_font_prop('tick', 'weight'))
                tick.set_fontstyle(self.get_font_prop('tick', 'style'))
                tick.set_fontsize(p['tick_size'])
        ...
        if self.legend_var.get() and self.plots_registry:
            ...
            plt.setp(leg.get_texts(), fontfamily=self.get_font_prop('legend', 'font'),
                     fontweight=self.get_font_prop('legend', 'weight'), fontstyle=self.get_font_prop('legend', 'style'))
    ...
    def export_style(self):
        ...
        if not path: return
        export_data = self.style_params.copy()
        for key, var in self.font_vars.items(): export_data[key] = var.get()
        export_data['show_legend'] = self.legend_var.get()
    ...
    def import_style(self):
        ...
        if not path: return
        try:
            with open(path, 'r', encoding='utf-8') as f: new_style = json.load(f)
            self.style_params.update(new_style)
            self.legend_var.set(self.style_params.get('show_legend', True))
            ...
            self.refresh_plots()
        except Exception as e: print(f"Ошибка импорта: {e}")
    ...
    def add_helper(self):
        x_l, y_l = self.ax.get_xlim(), self.ax.get_ylim(); is_v = self.h_type.get() == 'V'
        new_h = {'type': self.h_type.get(), 'pos': round(np.mean(x_l if is_v else y_l), 2), 'start': round(y_l[0] if is_v else x_l[0], 2), 'end': round(y_l[1] if is_v else x_l[1], 2), 'color': '#FF0000', 'width': 1.0, 'text': '', 'label_pos': 0.9, 'layer': 'Задний', 'font_size': 9}
        self.helper_lines.append(new_h); self.update_helper_ui(); self.refresh_plots()
    ...
    def update_helper_ui(self):
        ...
        for i, h in enumerate(self.helper_lines):
            ...
            for k, w in [('pos', 8), ('start', 8), ('end', 8)]:
                e = ttk.Entry(f_row, width=w); e.insert(0, str(h[k])); e.pack(side="left", padx=1); e.bind("<KeyRelease>", lambda ev, ix=i, key=k, en=e: self.edit_helper(ix, key, en.get()))
            e_txt = ttk.Entry(f_row, width=10); e_txt.insert(0, h['text']); e_txt.pack(side="left", padx=1); e_txt.bind("<KeyRelease>", lambda ev, ix=i, en=e_txt: self.edit_helper(ix, 'text', en.get()))
            for k, v_f, v_t in [('label_pos', 0, 100), ('font_size', 5, 30)]:
                val = int(h[k]*100) if k=='label_pos' else h[k]
                sp = tk.Spinbox(f_row, from_=v_f, to=v_t, width=4, command=lambda ix=i, key=k: self.edit_helper_spin(ix, key))
                sp.delete(0, "end"); sp.insert(0, str(val)); sp.pack(side="left", padx=1); h[f'{k}_spin'] = sp
            cb_l = ttk.Combobox(f_row, values=["Задний", "Передний"], width=8, state="readonly"); cb_l.set(h['layer']); cb_l.pack(side="left", padx=1); cb_l.bind("<<ComboboxSelected>>", lambda e, ix=i, c=cb_l: self.edit_helper(ix, 'layer', c.get()))
            sp_w = tk.Spinbox(f_row, from_=0.1, to=10.0, increment=0.1, width=4, command=lambda ix=i: self.edit_helper_spin(ix, 'width')); sp_w.delete(0, "end"); sp_w.insert(0, str(h['width'])); sp_w.pack(side="left", padx=1); h['width_spin'] = sp_w
    ...
    def update_ui_list(self):
        ...
        for i, item in enumerate(reversed(self.plots_registry)):
            ...
            ttk.Button(f, text="↓", width=2, command=lambda ix=idx: self.move_plot(ix, -1)).pack(side="left")
            btn_c = tk.Button(f, bg=item['color'], width=2, command=lambda ix=idx: self.pick_color(idx)); btn_c.pack(side="left", padx=2)
