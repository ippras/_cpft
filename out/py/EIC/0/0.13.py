...
class ChromatogramApp:
    ...
    def create_control_window(self):
        ...
        self.ctrl_win.geometry("500x650")
        ...
        lbl_files = ttk.Label(self.ctrl_win, text="Список файлов (Цвет | Имя в легенде):", font=('Arial', 10, 'bold'))
        ...
        self.canvas_files = tk.Canvas(self.files_container, height=200)
    ...
    def update_ui_list(self):
        """Пересоздает список файлов в окне настроек"""
        ...
        for i, item in enumerate(self.plots_registry):
            ...
            # Привязка изменения текста к обновлению легенды
    ...
    def on_label_change(self, index, var):
        """Вызывается при редактировании текста в Entry"""
        new_label = var.get()
        self.plots_registry[index]['label'] = new_label
        self.plots_registry[index]['line'].set_label(new_label)
        # Перерисовываем только легенду для производительности
    ...
    def pick_color(self, index):
        ...
        if color:
            ...
            self.update_ui_list() # Чтобы обновить цвет кнопки в списке
            self.refresh_plots()
