...
class ChromatogramApp:
    ...
    def setup_tab_labels(self):
        ...
        self.label_entries = {}
        labels_config = [
            ("Заголовок графика:", 'title_text'),
            ("Ось X (Время):", 'x_label'),
            ("Ось Y (Абсолютный):", 'y_label_abs'),
            ("Ось Y (Нормированный):", 'y_label_norm')
        ]

        for i, (txt, key) in enumerate(labels_config):
            ttk.Label(frame, text=txt).pack(anchor="w", pady=(10, 0))
            ent = ttk.Entry(frame)
            ent.insert(0, self.style_params[key])
    ...
    def reset_all_settings(self):
        ...
        for key, entry in self.label_entries.items():
            ...
            entry.insert(0, self.style_params[key])
    ...
    def update_style(self, event=None):
        ...
        self.style_params['show_legend'] = self.legend_var.get()
        ...
        if self.style_params['show_legend'] and self.plots_registry:
            leg = self.ax.legend(loc='upper right', fontsize=self.style_params['legend_size'])
            plt.setp(leg.get_texts(), fontsize=self.style_params['legend_size'])
