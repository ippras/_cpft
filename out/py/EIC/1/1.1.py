...
class ChromatogramApp:
    def __init__(self, root):
        ...
        # 2. Данные
        self.plots_registry = []
        self.helper_lines = []    # {type, pos, start, end, color, width, text, label_pos, layer}
        self.helper_artists = []  # Объекты на графике
    ...
    def create_control_window(self):
        ...
        self.ctrl_win.geometry("950x900")
    ...
    def setup_tab_helpers(self):
        ...
        # Заголовки списка
        h_header = ttk.Frame(frame, padding=(0, 5))
        h_header.pack(fill="x")
        cols = [("Тип", 40), ("Поз.", 70), ("Старт", 70), ("Конец", 70), ("Текст", 100), ("Метка %", 60), ("Слой", 80)]
        for t, w in cols:
    ...
    def update_helper_ui(self):
        ...
        for i, h in enumerate(self.helper_lines):
            f = ttk.Frame(self.h_frame); f.pack(fill="x", pady=2)
            
            ttk.Label(f, text=h['type'], width=3).pack(side="left")
            
            # Поля ввода: Поз, Старт, Конец
            for k, w in [('pos', 8), ('start', 8), ('end', 8)]:
                e = ttk.Entry(f, width=w); e.insert(0, str(h[k])); e.pack(side="left", padx=1)
                e.bind("<KeyRelease>", lambda ev, ix=i, key=k, en=e: self.edit_helper(ix, key, en.get()))
            
            # Текст
            e_txt = ttk.Entry(f, width=12); e_txt.insert(0, h['text']); e_txt.pack(side="left", padx=1)
            e_txt.bind("<KeyRelease>", lambda ev, ix=i, en=e_txt: self.edit_helper(ix, 'text', en.get()))
            
            # Позиция метки (Spinbox 0-100)
            sp_p = tk.Spinbox(f, from_=0, to=100, width=4, command=lambda ix=i: self.edit_helper_spin(ix, 'label_pos'))
            sp_p.delete(0, "end"); sp_p.insert(0, str(int(h['label_pos']*100))); sp_p.pack(side="left", padx=1)
            h['label_spin'] = sp_p
            
            # Слой
            cb_l = ttk.Combobox(f, values=["Задний", "Передний"], width=8, state="readonly")
            cb_l.set(h['layer']); cb_l.pack(side="left", padx=1)
            cb_l.bind("<<ComboboxSelected>>", lambda e, ix=i, c=cb_l: self.edit_helper(ix, 'layer', c.get()))

            # Толщина
            sp_w = tk.Spinbox(f, from_=0.1, to=5.0, increment=0.1, width=4, command=lambda ix=i: self.edit_helper_spin(ix, 'width'))
            sp_w.delete(0, "end"); sp_w.insert(0, str(h['width'])); sp_w.pack(side="left", padx=1)
            h['width_spin'] = sp_w
            
            # Цвет и Удаление
            btn_c = tk.Button(f, bg=h['color'], width=2, relief="flat", command=lambda ix=i: self.pick_helper_color(ix)); btn_c.pack(side="left", padx=2)
            ttk.Button(f, text="🗑️", width=3, command=lambda ix=i: self.remove_helper(ix)).pack(side="right")
    ...
    def edit_helper_spin(self, idx, key):
        try:
            if key == 'label_pos': self.helper_lines[idx][key] = float(self.helper_lines[idx]['label_spin'].get()) / 100.0
            if key == 'width': self.helper_lines[idx][key] = float(self.helper_lines[idx]['width_spin'].get())
            self.refresh_plots()
        except: pass
    ...
    def draw_helpers(self):
        ...
        for h in self.helper_lines:
            ...
            if h['type'] == 'V':
                ...
                # Считаем Y для текста
                ty = h['start'] + (h['end'] - h['start']) * h['label_pos']
                t = self.ax.text(h['pos'], ty, f" {txt}", color=h['color'], fontsize=9, va='center', ha='left', fontweight='bold', zorder=z+1)
            else:
                l = self.ax.plot([h['start'], h['end']], [h['pos'], h['pos']], color=h['color'], lw=h['width'], zorder=z)[0]
                # Считаем X для текста
                tx = h['start'] + (h['end'] - h['start']) * h['label_pos']
                t = self.ax.text(tx, h['pos'], f" {txt}", color=h['color'], fontsize=9, va='bottom', ha='center', fontweight='bold', zorder=z+1)
            
            self.helper_artists.extend([l, t])

    # --- ВСЕ ОСТАЛЬНЫЕ МЕТОДЫ (МАСШТАБ, ФАЙЛЫ, СТИЛЬ) ---
    def setup_tab_scale(self):
        ...
        for t_type in ['major', 'minor']:
            for ax_name in ['x', 'y']:
                f = ttk.Frame(frame); f.pack(fill="x")
                ttk.Label(f, text=f"{ax_name.upper()} ({t_type}):", width=12).pack(side="left")
                s = ttk.Scale(f, from_=0, to=(10 if ax_name=='x' else 1000000), command=lambda e, a=ax_name, t=t_type: self.on_tick_change(a, t))
                s.pack(side="left", fill="x", expand=True)
                e = ttk.Entry(f, width=10); e.pack(side="right", padx=5); e.insert(0, "0.0"); e.bind("<Return>", lambda ev, a=ax_name, t=t_type: self.on_tick_entry(a, t))
    ...
    def setup_tab_style(self):
        ...
        self.font_combo.set(self.style_params['font_family']); self.font_combo.grid(row=0, column=1, sticky="ew", columnspan=2); self.font_combo.bind("<<ComboboxSelected>>", self.update_style)
