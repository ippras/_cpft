...
class ChromatogramApp:
    def __init__(self, root):
        ...
        # 2. Данные
        self.plots_registry = []
        self.helper_lines = []    # Данные: {type, pos, lim, color, width}
    ...
    def create_control_window(self):
        ...
        self.ctrl_win.geometry("850x900")
    ...
    def setup_tab_helpers(self):
        frame = ttk.Frame(self.tab_helpers, padding=15); frame.pack(fill="both", expand=True)
        ...
        ttk.Label(frame, text="Список линий (Поз | Длина | Толщ | Цвет):", font='Arial 9 bold').pack(anchor="w", pady=(10,0))
    ...
    def add_helper(self):
        ...
        new_h = {
            ...
            'pos': round(np.mean(x_l if self.h_type.get()=='V' else y_l), 2),
            'lim': round(y_l[1] if self.h_type.get()=='V' else x_l[1], 2),
            'color': '#FF0000',
            'width': 1.0
        }
    ...
    def update_helper_ui(self):
        ...
        for i, h in enumerate(self.helper_lines):
            ...
            ttk.Label(f, text=h['type'], width=2).pack(side="left")
            
            # Позиция и Лимит
            for k in ['pos', 'lim']:
                e = ttk.Entry(f, width=8); e.insert(0, str(h[k])); e.pack(side="left", padx=2)
                e.bind("<KeyRelease>", lambda ev, ix=i, key=k, en=e: self.edit_helper(ix, key, en.get()))
            
            # Толщина
            sp = tk.Spinbox(f, from_=0.1, to=5.0, increment=0.1, width=4, command=lambda ix=i: self.edit_helper_width(ix))
            sp.delete(0, "end"); sp.insert(0, str(h['width'])); sp.pack(side="left", padx=2)
            h['width_spin'] = sp
            
            # Цвет и Удаление
            btn_c = tk.Button(f, bg=h['color'], width=2, command=lambda ix=i: self.pick_helper_color(ix)); btn_c.pack(side="left", padx=2)
            ttk.Button(f, text="🗑️", width=3, command=lambda ix=i: self.remove_helper(ix)).pack(side="right")

    def edit_helper(self, idx, key, val):
        try: self.helper_lines[idx][key] = float(val); self.refresh_plots()
        except: pass

    def edit_helper_width(self, idx):
        try: self.helper_lines[idx]['width'] = float(self.helper_lines[idx]['width_spin'].get()); self.refresh_plots()
        except: pass
    ...
    def setup_tab_scale(self):
        ...
        for t_type in ['major', 'minor']:
            for ax_name in ['x', 'y']:
                ...
                s = ttk.Scale(f, from_=0, to=limit, command=lambda e, a=ax_name, t=t_type: self.on_tick_change(a, t))
                s.pack(side="left", fill="x", expand=True)
    ...
    def refresh_plots(self):
        ...
        for h in self.helper_lines:
            w = h.get('width', 1.0)
            if h['type'] == 'V':
                l = self.ax.vlines(h['pos'], 0, h['lim'], colors=h['color'], lw=w, zorder=1)
                t = self.ax.text(h['pos'], h['lim'], f" {h['pos']}", color=h['color'], fontsize=9, va='bottom', ha='center', fontweight='bold')
            else:
                l = self.ax.hlines(h['pos'], self.ax.get_xlim()[0], h['lim'], colors=h['color'], lw=w, zorder=1)
                t = self.ax.text(h['lim'], h['pos'], f" {h['pos']}", color=h['color'], fontsize=9, ha='left', va='center', fontweight='bold')
    ...
    def add_files(self):
        ...
        for p in paths:
            ...
            if t is not None:
                ...
                line, = self.ax.plot(t, y, label=lbl, lw=self.style_params['line_width'], color=colors[len(self.plots_registry)%len(colors)])
    ...
    def update_ui_list(self):
        ...
        for i, item in enumerate(reversed(self.plots_registry)):
            ...
            ttk.Button(f, text="↓", width=2, command=lambda ix=idx: self.move_plot(ix, -1)).pack(side="left")
            tk.Button(f, bg=item['color'], width=2, command=lambda ix=idx: self.pick_color(ix)).pack(side="left", padx=2)
